...
export function CapabilityItem({ capability }: CapabilityItemProps) {
  return (
    <motion.div
      className="group flex items-start gap-3 py-2.5 cursor-default"
      whileHover={{ x: 4 }}
      transition={{ duration: 0.2 }}
    >
      <span className="w-1 h-1 rounded-full bg-foreground/20 group-hover:bg-accent transition-colors duration-200 mt-2.5 flex-shrink-0" />
